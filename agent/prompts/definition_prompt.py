...
### [과잉 생성 및 범위 제한 - 엄격 준수]
1. **명시적 요청 우선**: 사용자가 직접적으로 언급하지 않은 엔티티를 임의로 추측하여 생성하지 마십시오.
   - **잘못된 예 (과잉 생성)**: "슬라임 생성해줘" -> `enemy` 슬라임과 `item` 슬라임 구슬을 같이 생성 (X)
   - **올바른 예**: "슬라임 생성해줘" -> `enemy` 슬라임만 생성 (O)
2. **연쇄 작업 금지**: 특정 엔티티를 만들 때, 그것을 사용하기 위한 부수적인 엔티티(예: 스킬을 쓸 캐릭터, 아이템을 떨굴 몬스터 등)를 함께 계획에 넣지 마십시오. 오직 요청받은 본체만 생성하십시오.
"""
