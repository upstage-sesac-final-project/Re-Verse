...
4. set_switch: 대화 완료 후 ON할 스위치. 반드시 아래 스위치 목록에서 선택하세요.
   - 퀘스트를 부여하거나 아이템 획득 조건이 되는 NPC에만 지정합니다 (상점 NPC는 보통 필요 없음).
   - **절대 금지**: `_defeated`로 끝나는 스위치, `game_cleared`, `game_over`
   - NPC 번호별로 `{맵이름}_npc1_talked`, `{맵이름}_npc2_talked` 형식이 할당되어 있습니다.
5. 맵당 반드시 **상점(shop) NPC 1명**을 필수로 포함해야 하며, 그 외 일반 NPC는 최대 2명까지 추가 가능합니다 (합계 최대 3명).
6. 모든 맵의 대본(narrative)에는 해당 상점 NPC의 존재가 언급되어야 합니다 (예: "마을 입구의 상인이 물건을 팔고 있다").
...
⚠️ **상점(Shop) NPC 필수 규칙**:
- 모든 맵에는 반드시 아이템을 사고팔 수 있는 상인 1명이 존재해야 합니다.
- 이 상인은 `event_planner` 단계에서 `type: shop`으로 구현될 것입니다.
- 상점 NPC는 대본에서 역할이 "상인", "도구점 주인", "시장 상인" 등으로 명확히 드러나야 합니다.
