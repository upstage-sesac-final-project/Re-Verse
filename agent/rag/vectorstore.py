...
class SQLiteVectorStore:
    ...
    def add_documents(self, collection_name: str, ids: list[str], vectors: list[list[float]], documents: list[str], metadatas: list[dict[str, Any]]):
        """문서 추가"""
    ...
    def similarity_search(self, collection_name: str, query_vector: list[float], k: int = 5, where: dict[str, Any] | None = None) -> dict[str, Any]:
        """유사도 검색"""
        all_docs = []
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            # 메타데이터 필터링 (간단히 처리)
            cursor.execute("SELECT id, vector, content, metadata FROM documents WHERE collection_name = ?", (collection_name,))
