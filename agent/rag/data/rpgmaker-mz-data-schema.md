...
| classId | int | 직업 ID (Classes.json 참조) | `1` |
| initialLevel | int | 초기 레벨 | `25` |
| maxLevel | int | 최대 레벨 | `99` |
| profile | string | 프로필 설명 텍스트 | `"용감한 청년"` |
| characterName | string | 맵 스프라이트 파일명 (img/characters/) | `"Actor1"` |
| characterIndex | int | 스프라이트 시트 내 인덱스 (0~7) | `0` |
| faceName | string | 얼굴 이미지 파일명 (img/faces/) | `"Actor1"` |
| faceIndex | int | 얼굴 시트 내 인덱스 (0~7) | `0` |
| battlerName | string | SV 전투 스프라이트 파일명 (img/sv_actors/) | `"Actor1_1"` |
| equips | int[5] | 초기 장비 ID [무기, 방패, 머리, 몸, 장신구] | `[1, 1, 0, 0, 0]` |
...
| id | int | 고유 ID | `1` |
| name | string | 직업명 | `"검사"` |
| expParams | int[4] | 경험치 곡선 [기본값, 추가값, 증가율A, 증가율B] | `[30, 20, 30, 30]` |
| params | int[8][] | 레벨별 능력치 성장 곡선. 8개 능력치 x 레벨 수 | (2차원 배열) |
...
|--------|------|------|
| level | int | 습득 레벨 |
| skillId | int | 스킬 ID (Skills.json 참조) |
| note | string | 메모 |
...
| conditionParam1 | float | 조건 파라미터 1 |
| conditionParam2 | float | 조건 파라미터 2 |
| rating | int | 행동 우선도 (1~10, 높을수록 자주) |
...
| price | int | 가격 | `300` |
| wtypeId | int | 무기 유형 ID (System.weaponTypes 참조) | `2` |
| etypeId | int | 장비 유형: 항상 1=무기 | `1` |
| params | int[8] | 능력치 보정 [MHP, MMP, ATK, DEF, MAT, MDF, AGI, LUK] | `[0,0,10,0,0,0,0,0]` |
| traits | Trait[] | 특성 배열 | (배열) |
| animationId | int | 공격 시 애니메이션 ID | `6` |
| note | string | 메모 | `""` |
...
| price | int | 가격 | `100` |
| atypeId | int | 방어구 유형 ID (System.armorTypes 참조) | `1` |
| etypeId | int | 장비 슬롯: 2=방패, 3=머리, 4=몸, 5=장신구 | `4` |
| params | int[8] | 능력치 보정 [MHP, MMP, ATK, DEF, MAT, MDF, AGI, LUK] | `[0,0,0,5,0,0,0,0]` |
| traits | Trait[] | 특성 배열 | (배열) |
| note | string | 메모 | `""` |
...
| 필드명 | 타입 | 설명 | 예시 |
|--------|------|------|------|
| id | int | 고유 ID (1=기본공격, 2=방어) | `1` |
| name | string | 스킬명 | `"공격"` |
| description | string | 설명 | `""` |
| iconIndex | int | 아이콘 번호 | `76` |
| stypeId | int | 스킬 유형: 0=없음(기본), 1=마법, 2=필살기 등 | `0` |
| mpCost | int | 소모 MP | `0` |
| tpCost | int | 소모 TP | `0` |
...
| successRate | int | 성공률 (%) | `100` |
| repeats | int | 반복 횟수 | `1` |
| tpGain | int | 사용 시 획득 TP | `5` |
| hitType | int | 명중 유형: 0=확정, 1=물리, 2=마법 | `1` |
| animationId | int | 사용 시 애니메이션 ID (-1=없음) | `-1` |
| damage | Damage | 데미지 설정 | `{}` |
| effects | Effect[] | 효과 배열 | `[]` |
| message1 | string | 사용 메시지 1 (`%1` = 캐릭터명) | `"%1이(가) 공격합니다!"` |
| message2 | string | 사용 메시지 2 | `""` |
| messageType | int | 메시지 표시 유형 | `1` |
| requiredWtypeId1 | int | 필요 무기유형 1 (0=없음) | `0` |
| requiredWtypeId2 | int | 필요 무기유형 2 (0=없음) | `0` |
| note | string | 메모 | `""` |
...
| autoRemovalTiming | int | 자동 해제: 0=없음, 1=행동종료후, 2=턴종료후 | `0` |
| minTurns | int | 최소 지속 턴 | `1` |
| maxTurns | int | 최대 지속 턴 | `1` |
| removeByDamage | bool | 피해 시 해제 여부 | `false` |
| chanceByDamage | int | 피해 시 해제 확률 (%) | `100` |
| removeByWalking | bool | 걸을 때 해제 여부 | `false` |
| stepsToRemove | int | 해제까지 걸음 수 | `100` |
...
| 필드명 | 타입 | 설명 |
|--------|------|------|
| enemyId | int | 적 ID (Enemies.json 참조) |
| x | int | 전투 화면 X 좌표 |
| y | int | 전투 화면 Y 좌표 |
| hidden | bool | 등장 시 숨김 여부 |

### TroopPage 구조
| 필드명 | 타입 | 설명 |
|--------|------|------|
| conditions | object | 실행 조건 {turnA, turnB, turnEnding, ...} |
| list | Command[] | 이벤트 커맨드 리스트 |
| span | int | 범위: 0=전투, 1=턴, 2=모멘트 |
...
| timings | array | 레거시 타이밍 | `[]` |
| offsetX | int | X 오프셋 | `0` |
| offsetY | int | Y 오프셋 | `0` |
| rotation | object | 회전 {x, y, z} | `{x:0, y:0, z:0}` |
| scale | int | 스케일 (%) | `50` |
| speed | int | 재생 속도 (%) | `100` |
...
|--------|------|------|
| frame | int | 프레임 번호 |
| duration | int | 지속 시간 (프레임) |
| color | int[4] | [R, G, B, 강도] |

### SoundTiming 구조
| 필드명 | 타입 | 설명 |
|--------|------|------|
| frame | int | 프레임 번호 |
| se | object | {name, volume, pitch, pan} |
...
| 필드명 | 타입 | 설명 |
|--------|------|------|
| type | int | 0=없음, 1=HP데미지, 2=MP데미지, 3=HP회복, 4=MP회복, 5=HP흡수, 6=MP흡수 |
| elementId | int | 속성 ID (-1=보통공격, 0=없음, 1~=속성ID) |
