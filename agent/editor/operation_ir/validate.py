...
def validate_operation_tuples(ops: list[dict], extractions: list[dict]) -> tuple[bool, str]:
    ...
    for op in ops:
        ...
        subject_fully_missing = sid in (None, "", "NEW") and not sname and not is_system_field_op
        # 이름은 있지만 id 해소 실패 — resolve.py 를 거쳤는데도 id 가 None 이면 대상 미식별
        subject_unresolved = sid in (None, "", "NEW") and bool(sname) and not is_system_field_op
