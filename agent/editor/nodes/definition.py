...
from agent.editor.nodes.game_index_resolve import apply_index_resolution
...
# ──────────────────────────────────────────────
# Step 5: 코드 기반 operation IR 직접 생성
        ...
        if action == "CREATE":
            if sub_cat == "map":
                ...
                if ranked_map_candidates:
                    ...
                    logger.info(
                        "[Step5] 맵 추가 요청이지만 ranked_map_candidates 없음 → Step 5 fallback"
                    )
                    return []
            ops.append(
                {
