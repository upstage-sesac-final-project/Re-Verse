...
class TestSanitizeMzItemEffects:
    ...
    def _load_executor(self):
        self.executor_module = importlib.import_module("agent.editor.nodes.executor")
...
class TestFilterCategoryLabels:
    ...
    @pytest.fixture(autouse=True)
    def _load(self):
        from agent.editor.nodes.definition import filter_category_labels
...
# ──────────────────────────────────────────────────────────────
# resolve_mcp_server_key 우선순위 (구 test_mcp_server_resolve.py 에서 통합)
# ──────────────────────────────────────────────────────────────
