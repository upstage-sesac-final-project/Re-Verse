...
async def main():
    ...
    while True:
        try:
            ...
            # 초기 상태
            state = {"user_input": user_input, "game_id": game_id, "conversation_history": []}
            ...
            print(
                f"  [Router] 의도: {state.get('intent')} (신뢰도: {state.get('confidence', 0.0):.2f})"
            )

            # 만약 Router에서 즉시 응답이 나왔거나 (추가 정보 필요 등) 의도가 2번으로 갈 수 없는 경우
            if state.get("intent") not in ["게임_요소_생성", "게임_요소_수정", "게임_요소_조회"]:
                print(
                    f"  [Router Response]: {state.get('final_response', '진행할 수 없는 의도입니다.')}"
                )
                continue
            ...
            if not definition_output.get("params_sufficient"):
                ...
                for i, mod in enumerate(mods, 1):
                    ...
                    print(
                        f"    대상: {mod['target_entity']['category']} (ID: {mod['target_entity']['id']}, 이름: {mod['target_entity']['name']})"
                    )

                    if mod.get("action_object"):
                        print(
                            f"    객체: {mod['action_object']['category']} (ID: {mod['action_object']['id']}, 이름: {mod['action_object']['name']})"
                        )
                    ...
                    if action == "READ":
                        ...
                        print(
                            f"{field_info} (현재값: {mod.get('current_value')} -> 변경값: {mod.get('new_value')})"
                        )
                    elif action == "CREATE":
