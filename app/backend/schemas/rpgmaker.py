...
class EntityMapping(BaseModel):
    """엔티티 추출 및 ID 매핑 정보"""
    category: str = Field(..., description="데이터 카테고리 (예: Actors, Items, Skills, Classes 등)")
...
class DefinitionNodeResponse(BaseModel):
    ...
    file: str = Field(..., description="수정/조회할 대상 JSON 파일명 (예: Actors.json, Classes.json, Skills.json 등)")
    ...
    action_object: EntityMapping | None = Field(None, description="액션의 대상이 되는 보조 엔티티 (예: 부여할 스킬)")
    
    target_field: str | None = Field(None, description="수정할 구체적인 필드명 (예: traits, learnings, params[2])")
...
class FinalDefinitionResponse(BaseModel):
    ...
    modifications: list[DefinitionNodeResponse] = Field(..., description="수행해야 할 모든 수정 작업 목록 (순서대로 수행됨)")
    params_sufficient: bool = Field(True, description="요청 처리에 필요한 정보가 충분한지 여부")
    message_for_user: str | None = Field(None, description="정보가 부족하거나 사용자에게 질문이 필요할 때 작성")
    overall_thought_process: str = Field(..., description="전체적인 작업 계획 (예: 스킬 생성 후 캐릭터 traits에 연결)")
