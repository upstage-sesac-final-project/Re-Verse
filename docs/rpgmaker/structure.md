...
Re:Verse가 다루는 RPG Maker MZ 프로젝트 데이터의 경로·스키마 레퍼런스.
...
- `Actors.json`, `Classes.json`, `Enemies.json`, `Items.json`, `Weapons.json`, `Armors.json`, `Skills.json`, `States.json`

**시스템·구조 파일**

- `System.json` — 게임 전역 설정
- `MapInfos.json` — 맵 메타 목록
- `Map001.json` … `MapNNN.json` — 단일 맵 (events 배열 포함)
- `Troops.json`, `CommonEvents.json`, `Tilesets.json`, `Animations.json`

## Validator 지원 현황

- 핵심 엔티티 파일만 schema map에 직접 등록.
- 맵 파일은 `Map001.json` 패턴을 잡아 `MapFile` 스키마로 검증.
- `CommonEvents.json`, `MapInfos.json`, `Tilesets.json`, `Troops.json`은 현재 validator에서 unsupported schema일 수 있음.
